...
export class PostFormComponent<TModel, TBag, TPostFormResponseModel extends PostFormResponseModel> implements OnInit {
    ...
    onSubmit(): void {
        this.httpService.postForm<TPostFormResponseModel>(this.postUrl, this.model)
            .subscribe(response => {
                var responseModel = Object.assign(this.createDefaultPostFormResponseModel(), response);
                responseModel.result = Object.assign(new ResultResponseModel(), response.result);
